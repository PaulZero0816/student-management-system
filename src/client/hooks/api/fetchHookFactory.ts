...
const fetchHookFactory = <K, R>(
  ...
  const cache = noCache
    ? undefined
    : new LRU<string | number, Promise<R>>(cacheOptions);
  ...
  const useApiHook: UseHookType<K, R> = (params, hookOptions) => {
    ...
    const cacheKey = useMemo(
      () => (disableCache ? "no-cache" : convertParamsToKey(params)),
      [disableCache, params]
    );
    ...
    useEffect(() => {
      ...
      const coreFetch = async (swr?: boolean | R, forceRefresh?: boolean) => {
        if (params) {
          ...
          try {
            const fetchPromise =
              (!disableCache && !forceRefresh && cache?.get(cacheKey!)) ||
              fetchMethod(params!);
      ...
      const unsubscribeRefresh = subscribeListener(
        async (params: RefreshParams<K, R>) => {
    ...
    return [
      ...
      state.error,
      (swr?: boolean) =>
        params && triggerRefreshListener({ keys: params, swr }),
    ];
  };
